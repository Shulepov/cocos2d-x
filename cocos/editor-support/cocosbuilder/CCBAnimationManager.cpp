...
void CCBAnimationManager::moveAnimationsFromNode(Node* fromNode, Node* toNode)
{
    ...
    if(baseValueIter != _baseValues.end() && baseValueIter->first != toNode)
    {
...
//         toNode->retain();
    ...
    if (objIter != _objects.end() && objIter->first != toNode)
    {
    ...
    if (seqsIter != _nodeSequences.end() && seqsIter->first != toNode)
    {
